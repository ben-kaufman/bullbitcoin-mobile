...
void main() {
  ...
  test('maps only Ledger-supported derivation roles', () {
    ...
    final repeated = wallet.copyWith(
      publicDescriptor: 'wsh(or_d(pk($key/<0;1>/*),pk($key/<0;1>/*)))',
    );
    expect(
      LedgerWalletPolicyAdapter.fromWallet(
        repeated,
  ...
  test('allows missing origins for keys owned by another signer', () {
    final wallet = _policyWalletWithOriginlessCosigner();
  ...
  group('LedgerDeviceRepositoryImpl', () {
    ...
    test('requires the fingerprint and account xpub to match', () async {
      ...
      await expectLater(
        repository.registerWalletPolicy(_device, wallet: wallet),
        throwsA(
          isA<OperationFailedLedgerError>().having(
            (error) => error.message,
            'message',
            'LEDGER_ERROR_WALLET_SIGNER_MISMATCH',
          ),
        ),
      );
      verifyNever(
    ...
    test('persists the registration HMAC for the matched signer', () async {
      ...
      await repository.registerWalletPolicy(_device, wallet: wallet);
      ...
      await expectLater(
        ...
        throwsA(
          isA<OperationFailedLedgerError>().having(
            (error) => error.message,
            'message',
            'LEDGER_ERROR_BITCOIN_APP_UPDATE_REQUIRED',
          ),
        ),
      );
      ...
      await expectLater(
        ...
        throwsA(
          isA<OperationFailedLedgerError>().having(
            (error) => error.message,
            'message',
            'LEDGER_ERROR_UNSUPPORTED_WALLET_POLICY',
          ),
        ),
      );
    });
    ...
    test('signs with the stored wallet-policy HMAC', () async {
      ...
      expect(result, 'partially-signed');
    });
      ...
      await expectLater(
        ...
        throwsA(
          isA<OperationFailedLedgerError>().having(
            (error) => error.message,
            'message',
            'LEDGER_ERROR_WALLET_POLICY_NOT_REGISTERED',
          ),
        ),
      );
    });
    ...
    test('rejects a wallet-policy address mismatch', () async {
      ...
      await expectLater(
        repository.verifyWalletAddress(
          _device,
          wallet: wallet,
...
Wallet _policyWalletWithOriginlessCosigner() {
