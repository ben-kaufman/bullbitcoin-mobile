...
void main() {
  ...
  test('validates key-path signing while Tapleaf metadata remains', () {
  ...
  test('keeps key-path signer status when Tapleaf metadata remains', () {
    ...
    final inspected = datasource.inspectPsbt(
      signed.psbt,
      wallet: wallet,
  ...
  test('accepts a Taproot input with only its non-witness UTXO', () {
    final descriptor = _taprootKeyPathDescriptor(
      externalKey: signers.first.externalPublic,
      internalKey: signers.first.internalPublic,
    );
  ...
  test('validates signer status and both Taproot signing orders', () {
  ...
  test('rejects Tapleaf control blocks for another output key', () {
    final descriptor = _taprootScriptDescriptor(
      externalKeys: signers.map((signer) => signer.externalPublic).toList(),
      internalKeys: signers.map((signer) => signer.internalPublic).toList(),
    );
