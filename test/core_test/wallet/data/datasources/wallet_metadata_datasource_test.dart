...
void main() {
  ...
  test('stores and fetches ordered signers and descriptor keys', () async {
    ...
    expect(storedMetadata!.scriptType, isNull);
  ...
  test('stores one xpub at distinct descriptor origins', () async {
    ...
    expect(
      stored.signers.single.descriptorKeys.map((key) => key.derivationPath),
      ["m/48'/1'/0'/2'", "m/48'/1'/1'/2'"],
    );
  });
