...
void main() {
  ...
  group('BdkFacade two-path descriptors', () {
    ...
    test('accepts different two-path branches in one descriptor', () {
      ...
      expect(parsed.descriptor, contains('/<0;1>/*'));
      expect(parsed.descriptor, contains('/<2;3>/*'));
      expect(parsed.externalDescriptor, contains('/0/*'));
      ...
      final privateConventionalKey = signers.first.externalPrivate.replaceAll(
        '/0/*',
        '/<0;1>/*',
      );
      final privateRecoveryKey = signers.first.externalPrivate.replaceAll(
        '/0/*',
        '/<2;3>/*',
      );
      final wallet = BdkFacade.createEphemeralDescriptorWallet(
        descriptor:
            'wsh(or_d(pk($privateConventionalKey),'
            'and_v(v:older(10),pk($privateRecoveryKey))))',
        isTestnet: true,
      );
      try {
        expect(
          wallet
              .peekAddress(keychain: bdk.KeychainKind.external_, index: 0)
              .address,
          isNot(
            wallet
                .peekAddress(keychain: bdk.KeychainKind.internal, index: 0)
                .address,
          ),
        );
      } finally {
