...
sealed class BitcoinPolicyNode {
  ...
  bool canBeSatisfiedBy(bool Function(BitcoinPolicyKey key) hasSignature);

  bool containsSignatureMatching(bool Function(BitcoinPolicyKey key) matches);
}
...
final class BitcoinThresholdPolicyNode extends BitcoinPolicyNode {
  ...
  bool canBeSatisfiedBy(bool Function(BitcoinPolicyKey key) hasSignature) {
    ...
    for (final child in children) {
      if (child.canBeSatisfiedBy(hasSignature)) satisfied++;
      if (satisfied >= threshold) return true;
    }
    return false;
  }
  ...
  @override
  bool containsSignatureMatching(bool Function(BitcoinPolicyKey key) matches) =>
      children.any((child) => child.containsSignatureMatching(matches));
